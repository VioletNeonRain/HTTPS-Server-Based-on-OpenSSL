...
void freeRequest(struct Request* req){  //释放request请求所占空间
    if(req==NULL) 
        return;
    if(req->bodyptr!=NULL) 
        free(req->bodyptr);
...
void freeResponse(struct FinalResponse* res){  //释放response所占空间
    if(res==NULL) 
        return;
    if(res->data!=NULL) 
        free(res->data);
...
void freeFileinfo(struct FileInfo* fi){  //释放FileInfo所占空间
    if(fi==NULL) 
        return;
    if(fi->fpath!=NULL) 
        free(fi->fpath);
...
void freeBlock(struct Block* block){  //释放Block所占空间
    if(block==NULL) 
        return;
...
void printRequest(struct Request* req){  //打印request信息
    printf("\n----------------------------------------------\n[New Request]\n");
    printf("    method: %s\n",req->method);
    printf("    URL: %s\n",req->URL);
    printf("    URLlen: %ld\n",req->URLlen);
...
void printResponse(struct Block* block){  //打印response信息
    printf("\n----------------------------------------------\n[Response]\n");
    printf("%s\n",block->response->data);
    if(block->fileinfo!=NULL)
...
void sendResponse(struct Block* block, Socket sock){  //发送客户端请求文件
    if(block==NULL) 
        return;
